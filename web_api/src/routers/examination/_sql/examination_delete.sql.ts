...
export async function deleteExamination(examId: number, userId: number) {
  ...
  const { owner_id, exam_number } = await t.queryFirstRow<{ owner_id: number | null, exam_number: number }>(v.gen`UPDATE exam_paper_template SET exam_number=exam_number-1 WHERE id=${templateId} RETURNING owner_id, exam_number`);
