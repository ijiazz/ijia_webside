...
test("分页获取帖子列表", async function ({ api, ijiaDbPool }) {
  ...
  expect(list1.cursor_next).toBeTypeOf("string");

  const list2 = await getPostList(list1.cursor_next!, 11);
...
test("向前分页", async function ({ api, ijiaDbPool }) {
  ...
  const { cursor_prev } = await getPublicPostList();
  ...
  const list1 = await getPublicPostList({ cursor: cursor_prev ?? undefined, forward: true, number: 2 });
