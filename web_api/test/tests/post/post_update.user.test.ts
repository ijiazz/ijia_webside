...
import { ReviewStatus, updatePostConfigFormApi, updatePostContentFromApi } from "./utils/prepare_post.ts";
...
test("更新审核不通过的帖子，审核状态和审核数据不变", async function ({ api, ijiaDbPool }) {
  ...
  await Promise.all(keys.map((id) => updatePostContentFromApi(api, id, { content_text: "更新" }, alice.token)));
