...
test("删除审核通过的题目后，题目将归属于系统，并更新用户总题数", async function ({ api, publicDbPool }) {
  ...
  expect(await getUserQuestionCount(alice.id)).toBe(1);
  ...
  await expect(getUserQuestionCount(alice.id), "题目计数减 1").resolves.toBe(0);
});
...
test("删除审核不通过的题目，应直接删除题目和审核数据", async function ({ api, publicDbPool }) {
  const alice = await prepareUniqueUser("alice");
  const { question_id } = await createSampleQuestion(api, alice.token);
  ...
  await expect(getReviewCount(+reviewId)).resolves.toBe(0);
});
test("拒绝空用户题目的审核时，应直接删除题目和审核数据", async function ({ api, publicDbPool }) {
  const alice = await prepareUniqueUser("alice");
  const { question_id } = await createSampleQuestion(api, alice.token);
  await deleteQuestion(api, question_id, alice.token);
...
async function getQuestionByIdCount(questionId: number) {
...
async function getReviewCount(reviewId: number) {
  return dbPool.queryCount(
    select(["*"])
      .from("review")
      .where(`id=${v(reviewId)}`),
