...
test("已经提交的题目，再次提交应返回 409", async function ({ api, publicDbPool }) {
  ...
  const { question } = await nextExaminationQuestion(api, alice.token, examinationId);
  const index = question!.index;
  await answerExaminationQuestion(api, alice.token, examinationId, { index, answer: [0] });

  await expect(answerExaminationQuestion(api, alice.token, examinationId, { index, answer: [0] })).responseStatus(409);
});
...
test("提交未开始作答的题目，应返回 409", async function ({ api, publicDbPool }) {
  ...
  await startExamination(api, alice.token, examinationId);
  await expect(answerExaminationQuestion(api, alice.token, examinationId, { index: 1, answer: [0] })).responseStatus(
    409,
  );
});
...
test("交卷后，不能继续作答", async function ({ api, publicDbPool }) {
  ...
  await startExamination(api, alice.token, examinationId);
  const question = await nextExaminationQuestion(api, alice.token, examinationId);
  await answerExaminationQuestion(api, alice.token, examinationId, { index: question.question!.index, answer: [0] });
  await endExamination(api, alice.token, examinationId);
