...
test("交卷别人的考试，应返回 404", async function ({ api, publicDbPool }) {
  ...
  await expect(endExamination(bob.token, examination_id)).responseStatus(404);
});
test("结束未允许未开考的考试，应返回 409", async function ({ api, publicDbPool }) {
  ...
  await expect(endExamination(alice.token, examination_id)).responseStatus(409);
});
...
test("结束已结束的考试，应返回 409", async function ({ api, publicDbPool }) {
  ...
  await expect(endExamination(alice.token, examination_id), "结束已结束的考试，应返回 409").responseStatus(409);
});
