...
- Hono: 后端框架，类似于 Express，但更轻量；本项目运行在 Deno 上。
- @asla/yoursql(https://github.com/asnowc/yoursql): SQL
  生成工具库，提供链式调用来构建 SQL 语句，支持参数化查询。
...
1. 首先在 `web_dto/`
   中定义接口，并定义它们的输入输出类型（如果需要），并确保它们被正确导出。
2. 在 `web_api/src/routers/`
   下找到对应的路由文件，按照现有的路由组织结构添加新的接口实现。
3. 在接口实现中，优先使用 `@asla/yoursql` 来构建 SQL
   语句，避免直接拼接字符串或使用模板字符串来构造 SQL。
...
文件路径必须是 `web_api/src/routers/user/profile.get.ts` 或
`web_api/src/routers/user/profile/.get.ts`。

且 `web_api/src/routers/user` 目录下要有一个 mod.ts 文件，导出 user
目录下要公开的函数、路由、常量。 user 目录一般会有下面的结构：
...
-sql 以 "-" 或"\_" 开头是为了遵循 tanstack router 的约定,
以后可能直接文件文件名自动生成路由，表示这个文件或目录不是路由文件
